...
def add_admin(self):
    username = askstring("Add Admin", "Enter new admin username:")
    password = askstring("Add Admin", "Enter new admin password:")

    if username and password:
        hashed_password = generate_password_hash(password)
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.execute("INSERT INTO admin_accounts (username, password) VALUES (%s, %s)", (username, hashed_password))
        connection.commit()
        connection.close()
...
class AdminPanel:
    def __init__(self, root):
        ...
        add_admin_button = ttk.Button(self.root, text="Add Admin", command=self.add_admin)
...
def main_screen():
    ...
    ttk.Button(root, text="Add Admin", command=add_admin).pack(pady=10)  # Add Admin Button
    ...
    ttk.Button(root, text="Login", command=login).pack(pady=20)
    ttk.Button(root, text="Register", command=show_register_form).pack(pady=10)  # Add Register Button
    ...
    tk.Label(root, text="Register", font=FONT_TITLE, bg=COLOR_PRIMARY, fg="white").pack(fill="x", pady=10)
    ...
    ttk.Button(root, text="Back to Login", command=lambda: [root.destroy(), main_screen()]).pack(pady=10)
    root.mainloop()
